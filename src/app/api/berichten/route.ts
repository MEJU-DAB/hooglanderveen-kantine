...
export async function POST(req: NextRequest) {
  ...
  const { title, content = '', category = 'nieuws', active = true, ticker = true, image = null, duration = 10 } = body;
  ...
  const result = await db.execute({
    sql: 'INSERT INTO berichten (title, content, category, active, ticker, image, sort_order, duration) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    args: [title.trim(), content, category, active ? 1 : 0, ticker ? 1 : 0, image, maxOrder + 1, Number(duration)],
  });
